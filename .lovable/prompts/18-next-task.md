...
## This iteration (v1.22.0)
Completed the remaining single-slide import row in `ImportExportSubmenu`: validated single-slide JSON now merges into the imported deck store after the current slide, shifts later slide-number links safely, and reloads through the existing imported-manifest boot path.
## Iteration note (v1.23.0)
