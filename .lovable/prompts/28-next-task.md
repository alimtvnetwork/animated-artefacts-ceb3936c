...
## This iteration (v1.33.0)
Executed M-04 (no runtime error; the payload is the recurring driver). Decision: keep `TableSlide` UNCAPPED but steer authors to the density-capped `DataTableSlide` for ≤5×8 — added JSDoc `@remarks` to `src/slides/enums.ts` (TableSlide block) and the header comment in `src/slides/types/TableSlide.tsx`, and flipped M-04 to ✅ in `quality/audit/remediation-plan.md`. Remaining: M-01 (#32 ambiguity, needs user) + the separate pending plan `01-slide-system-export-llm-overhaul`. Closeout: saved this `28` snapshot, synced registry, bumped to v1.33.0.
