...
## This iteration (v1.25.0)
No application error present: `bunx vitest run src/test/contracts.test.ts` passes 14/14 and the Vite preview build is clean — the recurring "error" payload is the next-task driver prompt itself, not a runtime/build failure. Root cause of the only open inconsistency: the `.lovable/prompts/` registry chain advances per iteration but the version pin/release log had not yet been rolled forward for this cycle. Closed the cycle: saved this `20` snapshot, synced the registry, bumped to v1.25.0, and pinned the release notes.
