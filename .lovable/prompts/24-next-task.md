...
## This iteration (v1.29.0)
No application error: the repeated payload is the next-task driver itself. The actual defect was prompt-registry drift and misleading activation metadata: `.lovable/prompt.md` still pointed to snapshot `19` as latest and labeled `04-next-task.md` as `always-on`, while `.lovable/prompts.md` had advanced to `23`. Synced both registries, downgraded `04-next-task.md` to on-demand, and recorded the new snapshot so the prompt history is internally consistent again. Followed `.lovable/coding-guidelines.md`; `spec/coding-guidelines/` + `.lovable/seo-guidelines.md` absent, skipped silently.