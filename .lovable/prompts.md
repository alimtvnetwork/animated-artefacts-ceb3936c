...
| 17 | [17-next-task.md](./prompts/17-next-task.md) | `next task (v5)`, `next N steps or tasks`, `next task with number` | latest saved snapshot |

Snapshots `05`–`16` are preserved as historical per-iteration saves of the same recurring next-task prompt.
