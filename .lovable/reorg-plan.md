...
- **Batch 8 (95–100) ✔ — PLAN COMPLETE.** Final gate: stale-path grep clean (no refs to old `php/`, `assets/icons`, `assets/camera-2026`, `metrics/`, `reports/`, `readme.md`); `tsc -p tsconfig.app.json --noEmit` exit 0; `bun run build` exit 0 (✓ built in ~25s). Full suite 832 passing. Lint + the 4 failing tests are PRE-EXISTING (camera shape-cycle + slide-fixture + hardcoded-white + webcam-close drift) — none in any file touched by the reorg. Root reorganization done: convention-over-configuration layout, every top-level + spec/* folder documented, onboarding docs + memory synced.
