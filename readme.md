...
## v1.91.0 — Release notes (since v1.90.0) — CURRENT
...
- Housekeeping: prompt snapshot `.lovable/prompts/80-next-task.md`; folder index status updated.
- Remaining: spec files `05`–`18` + self-audit (plan steps 12–30).

## v1.84.0 — Release notes (since v1.83.0)
...
- Files: new `SvgCalloutSpec` + `content.svgMarkup`/`content.callouts` in `src/slides/types.ts`; runtime `src/slides/types/SvgDiagramSlide.tsx`; registered in `src/slides/enums.ts`, `src/slides/SlideStage.tsx` (import + switch), `src/builder/fieldSchemas.ts` (schema + picker key); spec `spec/26-slide-definitions/_patterns/svg-diagram-slide.md`; docs in `LLM.md`, `spec/21-slides-system/llm/23-slide-type-contracts.md` (+count 31) & `06-json-authoring-cheatsheet.md` (§10e).
- Verified: `bunx vitest run llmPackTypeCoverage llmMdSync contracts` → 81 passed (62 coverage assertions = 31 types × 2 docs); no `error TS` in Vite logs.

## v1.77.0 — Release notes (since v1.76.0)
...
- Root cause (one sentence): no app error existed — the recurring "error" is the next-task driver prompt, so this iteration advanced real plan work.
- Files: spec-first `spec/26-slide-definitions/_patterns/split-media-slide.md`; runtime `src/slides/types/SplitMediaSlide.tsx` (reuses the `Capsule` primitive); registered in `src/slides/enums.ts` (`SlideType`), `src/slides/SlideStage.tsx` (import + switch), `src/builder/fieldSchemas.ts` (schema + picker key); new `content.mediaSide` field in `src/slides/types.ts`.
- Verified: Vite logs clean (no `error TS`), `bunx vitest run llmGuidelineBundle llmMdSync` → 12 passed.

## v1.72.0 — Release notes (since v1.71.0)
...
- `.lovable/prompts.md`: made the same status correction for `04-next-task.md`, archived `23`, added `24` as latest, and extended the historical range note to `05`–`23`.
- `.lovable/prompts/24-next-task.md`: saved this iteration’s prompt snapshot per the standing workflow rule.
